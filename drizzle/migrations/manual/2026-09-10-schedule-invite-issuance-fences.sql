...
SET @siif_fence_checks_ok := (
  SELECT COUNT(*) = 10 AND SUM(check_manifest.ENFORCED = 'YES') = 10
    ...
    AND SUM(check_manifest.CONSTRAINT_NAME = 'chk_schedule_invite_issuance_material_shape' AND check_manifest.NORMALIZED_CLAUSE = '((state=''idle''andattempt_expires_atisnullandcode_nonceisnullandcode_pepper_key_idisnullandrecipient_binding_hashisnullandprovider_idempotency_keyisnullandprovider_request_fingerprintisnullandattempt_count=0)or(state<>''idle''andattempt_expires_atisnotnullandcode_nonceisnotnullandregexp_like(concat(code_nonce,''!''),''^[0-9a-f]{64}!$'',''c'')andcode_pepper_key_idisnotnullandregexp_like(concat(code_pepper_key_id,''!''),''^[0-9a-f]{64}!$'',''c'')andrecipient_binding_hashisnotnullandregexp_like(concat(recipient_binding_hash,''!''),''^[0-9a-f]{64}!$'',''c'')andprovider_idempotency_keyisnotnullandregexp_like(concat(provider_idempotency_key,''!''),''^[0-9a-f]{64}!$'',''c'')andprovider_request_fingerprintisnotnullandregexp_like(concat(provider_request_fingerprint,''!''),''^[0-9a-f]{64}!$'',''c'')andattempt_countbetween1andmax_attempts))') = 1
    ...
    AND SUM(check_manifest.CONSTRAINT_NAME = 'chk_schedule_invite_issuance_failure_shape' AND check_manifest.NORMALIZED_CLAUSE = '((statein(''provider_unknown'',''provider_rejected'',''provider_accepted_activation_failed'')andfailure_codeisnotnullandregexp_like(concat(failure_code,''!''),''^[a-z][a-z0-9_]{0,63}!$'',''c''))or(statenotin(''provider_unknown'',''provider_rejected'',''provider_accepted_activation_failed'')andfailure_codeisnull))') = 1
    ...
    AND SUM(check_manifest.CONSTRAINT_NAME = 'chk_schedule_invite_issuance_provider_correlation' AND check_manifest.NORMALIZED_CLAUSE = '((statein(''provider_accepted'',''active'',''provider_accepted_activation_failed'')and(provider_correlation_idisnullorregexp_like(concat(provider_correlation_id,''!''),''^[a-za-z0-9._:-]{1,128}!$'',''c'')))or(statenotin(''provider_accepted'',''active'',''provider_accepted_activation_failed'')andprovider_correlation_idisnull))') = 1
...
SET @siij_checks_ok := (
  SELECT COUNT(*) = 4 AND SUM(check_manifest.ENFORCED = 'YES') = 4
    ...
    AND SUM(check_manifest.CONSTRAINT_NAME = 'chk_schedule_invite_issuance_journal_reason' AND check_manifest.NORMALIZED_CLAUSE = '((eventin(''attempt_superseded'',''provider_rejected'',''provider_unknown'',''activation_failed'')andreason_codeisnotnullandregexp_like(concat(reason_code,''!''),''^[a-z][a-z0-9_]{0,63}!$'',''c''))or(eventnotin(''attempt_superseded'',''provider_rejected'',''provider_unknown'',''activation_failed'')andreason_codeisnull))') = 1
    AND SUM(check_manifest.CONSTRAINT_NAME = 'chk_schedule_invite_issuance_journal_correlation' AND check_manifest.NORMALIZED_CLAUSE = '(provider_correlation_idisnullor(event=''provider_accepted''andregexp_like(concat(provider_correlation_id,''!''),''^[a-za-z0-9._:-]{1,128}!$'',''c'')))') = 1
    AND SUM(check_manifest.CONSTRAINT_NAME = 'chk_schedule_invite_issuance_journal_activation' AND check_manifest.NORMALIZED_CLAUSE = '((event=''activated''andschedule_invite_idisnotnull)or(event<>''activated''andschedule_invite_idisnull))') = 1
  FROM (
    SELECT table_constraints.CONSTRAINT_NAME, table_constraints.ENFORCED,
      REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER(check_constraints.CHECK_CLAUSE), CHAR(96), ''), '_utf8mb4', ''), ' ', ''), CHAR(10), ''), CHAR(13), '') AS NORMALIZED_CLAUSE
    FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS table_constraints

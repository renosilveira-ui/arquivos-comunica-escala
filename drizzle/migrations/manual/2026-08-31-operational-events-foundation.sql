-- 2026-08-31 — foundation corporativa de eventos operacionais.
--
...
-- conta ou convite nominal já persistidos e o e-mail será resolvido no worker
-- futuro, após revalidação de confiança.

-- As FKs compostas do evento para turno/alocação exigem chaves-pai com a
-- topologia completa. `id` já é PK; estas chaves apenas tornam a combinação
-- explicitamente referenciável e impedem cruzamento entre setores.
SET @idx_exists := (
...
-- O destinatário de convite usa uma FK composta com a instituição do evento.
-- A chave-pai é garantida aqui para não depender de ordem implícita entre
-- migrations manuais.
SET @idx_exists := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
...
-- A FK composta USER -> instituição exige uma chave-pai única. A versão
-- anterior do schema já declara essa unicidade, mas deployments legados podem
-- tê-la com nome gerado pelo banco; por isso a guarda verifica as colunas, e
-- não só um nome de índice específico.
SET @idx_exists := (
  SELECT COUNT(*)
  FROM (
    SELECT INDEX_NAME

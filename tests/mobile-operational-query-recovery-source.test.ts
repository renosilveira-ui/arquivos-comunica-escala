...
describe("contrato de recuperação somente nas telas operacionais", () => {
  ...
  it("não mostra candidatura vazia quando uma das fontes falha", () => {
    expect(applications).toContain("applicationsHasError || vacancyRequestsHasError");
  ...
  it("não converte falha de escopo profissional em filtros padrão", () => {
    ...
    expect(vacancies).toContain("title=\"Não foi possível carregar os filtros de vagas\"");
  });

  it("mantém arrays vazios estáveis e não deriva defaults da identidade das listas", () => {
    ...
    expect(filterDefaults).toContain("void options");
    expect(filterDefaults).toContain("[managerScope, isLoading]");
    expect(filterDefaults).not.toContain("[managerScope, isLoading, hospitals, sectors]");
  });
  ...
  it("não exibe lista ou botões stale sem resposta resolvida ou após revogação", () => {
    ...
    expect(vacancies).toContain("presentQueryError(professionalError).kind === \"ACCESS\"");
    expect(vacancies).toContain("presentQueryError(hospitalsError).kind === \"ACCESS\"");
    expect(vacancies).toContain("presentQueryError(sectorsError).kind === \"ACCESS\"");
  });
  ...
  it("não afirma contagem ou métricas cacheadas durante erro ou resposta pendente", () => {
    ...
    expect(vacancies).toContain("Aguardando a confirmação dos plantões em aberto…");
  });
});

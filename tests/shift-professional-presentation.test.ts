...
describe("apresentação da equipe de um plantão", () => {
  ...
  it("mostra a equipe completa quando o plantão próprio tem mais de uma pessoa", () => {
    expect(
      shiftProfessionalNameLines(
        {
          professionalNames: ["Nome da pessoa", "Colega de plantão"],
          isMine: true,
        },
        "Sem profissional",
      ),
    ).toEqual(["Nome da pessoa", "Colega de plantão"]);
  });

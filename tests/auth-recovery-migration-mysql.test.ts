...
describe("migration de recuperação de credenciais em MySQL isolado", () => {
  ...
  beforeAll(async () => {
    ...
    expect(String(rows[0]?.version)).toMatch(/^8\.0\.46(?:\D|$)/);
  });
  ...
  afterEach(async () => {
    try {
      ...
      // Só o prefixo allowlisted é removível: garante que um databaseName
      // inesperado derrube o teste em vez de dropar um schema alheio.

...
async function expireIfDue(
  ...
  const expired = await expireOpenRow(db, row.id, row.userId, now);
  if (expired) {
...
export async function expireWhatsAppPendingIntent(
  ...
  try {
    const updated = await expireOpenRow(db, id, userId, now);
    const latest = await loadByIdForUser(db, id, userId);
    if (!latest) return { ok: false, code: "NOT_FOUND" };
    if (!updated) {
      return isWhatsAppPendingTerminalStatus(latest.status)
        ? { ok: true, outcome: "already_terminal", row: latest }
        : { ok: true, outcome: "not_due", row: latest };
    }
    logSafe({
      event: "whatsapp_pending_expired",
      ...technicalLogFields(latest),
...
export async function cancelWhatsAppPendingIntent(
  ...
  try {
    const updated = await cancelOpenRow(db, id, userId, now);
    const latest = await loadByIdForUser(db, id, userId);
    if (!latest) return { ok: false, code: "NOT_FOUND" };
    if (!updated) {
      return { ok: true, outcome: "already_terminal", row: latest };
...
export async function clearExpiredWhatsAppPendingIntents(
  ...
  const db = await getDb();
  if (!db) return { expired: 0, payloadsCleared: 0 };
  ...
  logSafe({
    ...
    expired,
    payloadsCleared: expired + leftover,
  });
  return { expired, payloadsCleared: expired + leftover };
}

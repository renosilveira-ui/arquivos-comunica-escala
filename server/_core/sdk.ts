...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    // Test mode: read x-test-user-id header (priority 1)
    if (process.env.NODE_ENV === "development") {
      const testUserIdHeader = req.headers["x-test-user-id"] || req.headers["X-Test-User-Id"];
      if (typeof testUserIdHeader === "string" && testUserIdHeader.trim()) {
        const testUserId = testUserIdHeader.trim();
        const dbInstance = await getDb();
        if (dbInstance) {
          const [testUser] = await dbInstance.select().from(users).where(eq(users.id, Number(testUserId)));
          if (testUser) {

 ...
 * Visível apenas em desenvolvimento com flag explícita.
 */
export function DiagnosticBadge() {
  const testUserId = useTestUserId();
  const { user } = useAuth();

  if (Platform.OS !== "web" || !isTestModeEnabled()) {
    return null;
  }
  ...
  return (
    <View className="bg-yellow-500 p-3 mb-2 rounded-lg">
      <Text className="text-xs font-bold text-black mb-2">
        🩺 DIAGNÓSTICO DO MODO DE TESTE
      </Text>

      <View className="gap-1">
        <Text className="text-xs text-black">
          <Text className="font-bold">enableTestMode:</Text> ✅ true
        </Text>

        <Text className="text-xs text-black">
          <Text className="font-bold">testUserId:</Text>{" "}
          {testUserId !== null ? `✅ ${testUserId}` : "❌ none"}
        </Text>
        ...
        <Text className="text-xs text-black">
          ...
          {professional
            ? `✅ ${professional.name} (${professional.userRole})`
            : "❌ none"}
        </Text>
        ...
        <Text className="text-xs text-black">
          <Text className="font-bold">NEXT_PUBLIC_ENABLE_TEST_MODE:</Text>{" "}
          {nextPublicEnabled || "undefined"}
        </Text>

        <Text className="text-xs text-black">
          <Text className="font-bold">EXPO_PUBLIC_ENABLE_TEST_MODE:</Text>{" "}
          {expoPublicEnabled || "undefined"}
        </Text>
      </View>
    </View>
  );
